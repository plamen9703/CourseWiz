CREATE OR REPLACE FUNCTION users.get_user_student_login_by_email(
...
AS $BODY$
BEGIN
	IF in_user_email IS NULL THEN
		RAISE EXCEPTION 'user provided email can not be null or empty!';
	END IF;
	RETURN QUERY
		SELECT
    u.id ,
...
WHERE (in_user_email IS NOT NULL AND in_user_email = u.email);
END;
$BODY$;