CREATE OR REPLACE FUNCTION users.get_user_student_login_by_username(
...
BEGIN
	IF in_user_username IS NULL THEN
			RAISE EXCEPTION 'Provided username cannot be null!';
	END IF;
	RETURN QUERY
		SELECT
    u.id ,
...
WHERE
(in_user_username IS NOT NULL AND in_user_username = u.username);
END;
$BODY$;