CREATE OR REPLACE FUNCTION users.get_user_student_profile_by_identifier(
	in_user_id integer,
	in_user_username character varying,
	in_user_email character varying)
    RETURNS TABLE(user_id integer, user_username character varying, user_email character varying, user_roles character varying[], user_permissions character varying[], student_pin character varying, student_first_name character varying, student_last_name character varying, student_created_at timestamp without time zone)
    LANGUAGE 'plpgsql'
    COST 100
...
JOIN coursera.students s ON up.student_pin = s.pin
LEFT JOIN LATERAL (
